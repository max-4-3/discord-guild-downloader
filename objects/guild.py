...
class Guild:
    ...
    async def __make_req(self, **kwargs):
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(
                url, headers=self.__headers__, params={"with_counts": "true"}
            ) as response:
    ...
    def _init_(self, data: dict):
        ...
        self.owner = Owner(
            self.__loop__, int(str(data.get("owner_id") or -1)), self.__headers__
        )
...
class PartialGuilds:
    ...
    async def __list_guilds(self):
        async with aiohttp.ClientSession() as session:
            async with session.get(
                BASE + "/users/@me/guilds", headers=self.__headers__
            ) as response:
                return await response.json()
